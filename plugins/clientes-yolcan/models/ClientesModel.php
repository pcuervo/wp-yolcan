...
class ClientesModel {
	...
	public function getCountClientesPorClub(){
		return $this->_wpdb->get_results( "SELECT club_id, count(club_id) as total FROM (SELECT * FROM sy_usermeta WHERE meta_key = 'sy_capabilities' AND meta_value LIKE '%cliente%') as um
			INNER JOIN sy_opciones_clientes as oc
			ON um.user_id = oc.cliente_id
			GROUP BY club_id;", 
	 	 OBJECT );
	}
